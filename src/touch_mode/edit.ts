...
///// Make it look nice while being clicked on :)
...
import { ALGEBRAIC_INPUT_PROMPT } from '../input_box/algebraic_input.js';
...
async function editStackFrame(manager: InputBoxManager, index: number): Promise<void> {
  ...
  const text = await manager.show(new FreeformInputMethod(ALGEBRAIC_INPUT_PROMPT), currentStackValue);
  if (text) {
